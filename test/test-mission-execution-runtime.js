...
async function main() {
  ...
  await fs.writeFile(
    ...
    [
      ...
      '  require_deadline_for_priorities:',
      '    - critical',
      '  require_labels_for_retry_classes:',
      '    transient:',
      '      - runtime',
      '    policy:',
      '      - governance',
      '  require_dependencies_for_retry_classes:',
      '    policy:',
      '      - policy-baseline',
      'budgets:',
      '  max_total_selections: 16',
  ...
  const metadataDeniedPath = path.join(projectDir, 'mission-metadata-denied.yaml');
  await fs.writeFile(
    metadataDeniedPath,
    [
      'id: mission-metadata-denied',
      'title: Metadata denied claim',
      'status: ready',
      'tracker: local',
      'owner: platform-ops',
      'priority: high',
  ...
  await assert.rejects(
    claimWorkItem(metadataDeniedPath, { projectDir }),
    /Mission label "unknown-label".*Mission dependency "missing-dependency".*Mission retry class "policy" requires at least one label: governance/s,
  );
