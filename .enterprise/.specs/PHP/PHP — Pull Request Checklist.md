# PHP — Pull Request Checklist
## PHP / Laravel & Symfony — Gold Standard

> Mandatory for all PHP pull requests.
...
- [ ] Domain layer has zero framework dependencies (no Laravel/Symfony annotations, facades).
- [ ] Domain layer has zero Eloquent/Doctrine imports.
...
- [ ] No DB writes inside Query handlers.
...
- [ ] Eloquent models live in Infrastructure — not in Domain.
- [ ] Domain objects and Eloquent models mapped explicitly (no shared types).
- [ ] Migration script added for schema changes.
...
- [ ] No direct Guzzle `Client` or Symfony `HttpClient` usage outside the package.
...
- [ ] No exception used for control flow.
...
- [ ] New dependencies reviewed for CVEs (`composer audit`).

---

## 8. Testing

- [ ] Domain logic covered by unit tests (PHPUnit) — no framework bootstrap needed.
- [ ] Use case handlers covered by unit tests with mocked ports.
- [ ] Infrastructure adapters covered by integration tests.
- [ ] API layer covered by Laravel HTTP tests or Symfony WebTestCase.
...
- [ ] Deptrac / PHPArkitect architecture tests pass.
- [ ] Naming follows PHP profile of Naming & Conventions Standard.
- [ ] PHPDoc on all new public classes and methods.
...
- [ ] I confirm this PR fully complies with the PHP Architecture Standard, FR, NFR.
