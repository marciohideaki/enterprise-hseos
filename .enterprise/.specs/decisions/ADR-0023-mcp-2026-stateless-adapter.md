...
6. Legacy `initialize` support is isolated behind an explicit compatibility adapter with telemetry and a removal date; it cannot affect modern request semantics.
...
- Keep the legacy adapter available but disabled by default after the compatibility window.
