...
<critical>The workflow execution engine is governed by: {project-root}/.hseos/core/tasks/workflow.xml</critical>
<critical>You MUST have already loaded and processed: {project-root}/.hseos/hsm/workflows/4-implementation/sprint-status/workflow.yaml</critical>
...
Run `/hseos:bmm:workflows:sprint-planning` to generate it, then rerun sprint-status.</output>
...
- IF any story has status "review": suggest `/hseos:bmm:workflows:code-review`
- IF any story has status "in-progress" AND no stories have status "ready-for-dev": recommend staying focused on active story
- IF all epics have status "backlog" AND no stories have status "ready-for-dev": prompt `/hseos:bmm:workflows:create-story`
...
**Next Recommendation:** /hseos:bmm:workflows:{{next_workflow_id}} ({{next_story_id}})
...
Choice:</ask>
  ...
  <check if="choice == 1">
    <output>Run `/hseos:bmm:workflows:{{next_workflow_id}}`.
