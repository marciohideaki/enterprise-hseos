...
`@hseos/agent-runtime-contracts` is the vendor-neutral inward boundary of the HSEOS Agent Kernel. It contains strict data schemas, provider capability negotiation and runtime checks for the three canonical ports. It contains no model implementation, provider SDK, network client, persistence adapter or operational activation.
...
Every object schema is strict and requires `schema_version: 1`. Unknown fields and unsupported versions fail closed. Secret values are not contract fields; provider manifests may contain unique named references with an explicit secret-source URI only.
