...
`@hseos/agent-runtime-contracts` is the vendor-neutral inward boundary of the HSEOS Agent Kernel. It contains strict data schemas, provider capability negotiation and runtime checks for the canonical ports. It contains no model implementation, provider SDK, network client, persistence adapter or operational activation.
...
- `CompactionProvider`: assesses pressure and emits bounded lineage-bound history or tool-result replacements.
- `CheckpointProvider`: stores and reads immutable derivative checkpoint payloads without becoming event authority.
- `SubagentProvider`: forks, joins, cascade-cancels and disposes authority-bounded child sessions.
- `WorkflowEngine`: executes finite `parallel` or `pipeline` phases with durable child-lineage checkpoints.
...
Compaction contracts bind replacements to a SHA-256 digest of the exact ordered source messages, byte/message accounting, provider and compaction identity. History summaries remain plain assistant messages; tool pruning must preserve one ordered tool message for every original `tool_call_id`. The relational event embeds replacements and source identifiers, so checkpoint availability is never required to reconstruct a recorded request.

Orchestration contracts require globally unique workflow phase, step and child identities, a finite join deadline and explicit parallelism. Durable reservation/release facts consume the parent step budget before child dispatch; phase checkpoints then bind exact completed lineage without serving as the budget reservation. Resuming an active reservation requires its current `resume_from_ref` after the durable claim lease expires; `workflow.reclaimed` atomically rotates that claim so live, concurrent or stale resumers fail before dispatch. Checkpoints and releases carry the captured claim reference and are fenced again by replay. Completed workflow results must match the exact ordered phase/child declaration. Provider, parent, request and child correlation is checked before an orchestration result crosses the port boundary.
