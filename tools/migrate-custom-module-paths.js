...
/**
 * Find HSEOS directory in project
 */
function findHseosDir(projectDir = process.cwd()) {
  const possibleNames = ['.hseos'];

  for (const name of possibleNames) {
    const hseosDir = path.join(projectDir, name);
    if (fs.existsSync(hseosDir)) {
      return hseosDir;
    }
  }
...
async function migrate(directory) {
  ...
  const hseosDir = findHseosDir(projectRoot);

  if (!hseosDir) {
    console.error(chalk.red('✗ No HSEOS installation found in directory'));
    process.exit(1);
  }

  console.log(chalk.blue.bold('🔄 HSEOS Custom Module Path Migration'));
  console.log(chalk.dim(`Project: ${projectRoot}`));
  console.log(chalk.dim(`HSEOS Directory: ${hseosDir}`));
