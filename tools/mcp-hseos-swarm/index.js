...
function createServer() {
  return http.createServer((req, res) => {
    if (req.method === 'GET' && req.url === '/health') {
      ...
      res.end(JSON.stringify({ status: 'ok', server: 'hseos-swarm', tools: tools.size }));
      return;
    }
...
server.listen(port, '127.0.0.1', () => {
  console.log(`[swarm] MCP server listening on http://127.0.0.1:${port}`);
  console.log(`[swarm] Tools loaded: ${tools.size}`);
});
