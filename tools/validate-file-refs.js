 ...
 *
 * Validates cross-file references in HSEOS source files (agents, workflows, tasks, steps).
 * Catches broken file paths, missing referenced files, and absolute path leaks.
 *
 * What it checks:
 * - {project-root}/.hseos/ references in YAML and markdown resolve to real src/ files
...
// Pattern: {project-root}/.hseos/ references
const PROJECT_ROOT_REF = /\{project-root\}\/.hseos\/([^\s'"<>})\]`]+)/g;

// Pattern: {.hseos}/ shorthand references
const HSEOS_SHORTHAND_REF = /\{.hseos\}\/([^\s'"<>})\]`]+)/g;
...
function mapInstalledToSource(refPath) {
  // Strip {project-root}/.hseos/ or {.hseos}/ prefix
  let cleaned = refPath.replace(/^\{project-root\}\/\.hseos\//, '').replace(/^\{\.hseos\}\//, '');

  // Also handle bare .hseos/ prefix (seen in some invoke-task)
  cleaned = cleaned.replace(/^\.hseos\//, '');
...
function isInstallOnly(cleanedPath) {
  // Skip paths that only exist in the installed .hseos/ structure, not in src/
...
function extractYamlRefs(filePath, content) {
  ...
  function checkValue(value, range, keyPath) {
    ...
    // Check for {project-root}/.hseos/ refs
    const prMatch = value.match(/\{project-root\}\/.hseos\/[^\s'"<>})\]`]+/);
    if (prMatch) {
      refs.push({ file: filePath, raw: prMatch[0], type: 'project-root', line, key: keyPath });
    }

    // Check for {.hseos}/ refs
    const bmMatch = value.match(/\{.hseos\}\/[^\s'"<>})\]`]+/);
    if (bmMatch) {
...
function extractMarkdownRefs(filePath, content) {
  ...
  // {project-root}/.hseos/ refs
  runPattern(PROJECT_ROOT_REF, 'project-root');

  // {.hseos}/ shorthand
  runPattern(HSEOS_SHORTHAND_REF, 'project-root');
...
function resolveRef(ref) {
  ...
  if (ref.type === 'exec-attr') {
    ...
    if (execPath.includes('{.hseos}')) {
      return mapInstalledToSource(execPath);
    }
    if (execPath.startsWith('.hseos/')) {
      return mapInstalledToSource(execPath);
    }
    // Relative exec path
  ...
  if (ref.type === 'invoke-task') {
    ...
    const prMatch = ref.raw.match(/\{project-root\}\/.hseos\/([^\s'"<>})\]`]+)/);
    if (prMatch) return mapInstalledToSource(prMatch[0]);

    const bmMatch = ref.raw.match(/\{.hseos\}\/([^\s'"<>})\]`]+)/);
    if (bmMatch) return mapInstalledToSource(bmMatch[0]);

    const bareMatch = ref.raw.match(/.hseos\/([^\s'"<>})\]`]+)/);
