  ...
  enqueue(request) {
    if (!request || typeof request !== 'object' || Array.isArray(request) || typeof request.tool !== 'string') {
      ...
      return rejectedHandle(
        request,
        new ExecutionSchedulerError('Scheduler request must identify a tool', 'EXECUTION_REQUEST_INVALID'),
      );
    }
    if (!this.accepting) {
      this.metrics.rejected += 1;
      return rejectedHandle(
        request,
        new ExecutionSchedulerError('Scheduler is closed', 'EXECUTION_SCHEDULER_CLOSED'),
      );
    }
    if (
    ...
    void Promise.resolve()
      .then(async () => {
        ...
        if (outcome.ok || outcome.error.code !== 'EXECUTION_CANCELLED') {
          throw new ExecutionSchedulerError(
            'Governed port did not persist queued cancellation',
            'EXECUTION_CANCELLATION_INVALID',
          );
        }
        return outcome;
      })
...
module.exports = { ExecutionSchedulerError, GovernedExecutionScheduler };
