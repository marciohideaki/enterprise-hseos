  ...
  async execute(request) {
    ...
    try {
      ...
      try {
        ...
        throw new GovernedExecutionError('Provider registry lookup failed', 'EXECUTION_PROVIDER_RESOLUTION_FAILED', { cause: error.message });
      }
      ...
      if (request.signal && request.signal.aborted && contract.cancellation_policy === 'cooperative') {
        ...
        this._reconcile(warnings);
        return failureEnvelope(new GovernedExecutionError('Execution cancelled', 'EXECUTION_CANCELLED'), operationId, approvalEvidence, warnings);
      }
      ...
      try {
        ...
        const validReceiptRef =
          typeof providerFailure.receipt_ref === 'string' && providerFailure.receipt_ref.length > 0
            ? providerFailure.receipt_ref
            : null;
        ...
        const payloadByType = {
          ExecutionCancelled: { reason: normalized.message, phase: 'dispatch', ...(validReceiptRef ? { provider_receipt_ref: validReceiptRef } : {}) },
        ...
        if (persistenceError) {
          return this._terminalPersistenceFailure(
            operationId,
            [...approvalEvidence, ...receiptRefs],
            warnings,
            persistenceError,
          );
        }
      ...
      try {
        ...
        const persistenceError = this._persistOutcome(
          ...
          uncertain ? 'ExecutionOutcomeUncertain' : 'ExecutionFailed',
          uncertain
            ? { reason: 'provider completed but output schema validation failed', ...(providerResult.receipt_ref ? { provider_receipt_ref: providerResult.receipt_ref } : {}) }
            : { error: { code: error.code, message: error.message, retryable: false } },
