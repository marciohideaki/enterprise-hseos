...
description: Corrects workflow, agent, and command references in HSEOS documentation
---

# Fix Documentation References

> **Provenance note (2026-09-05):** recovered from a deleted, never-merged branch. The
> `hseos-help` workflow this document names as the "modern replacement" for
> `workflow-init`/`workflow-status` was not found in the current `.hseos/workflows/registry.yaml`
> — that specific claim may be stale. The reference-pattern-fixing methodology below (audience
> detection, backtick formatting, the self-check) was not otherwise re-verified against the
...
Fix incorrect references to workflows, agents, and commands in HSEOS documentation files.
...
**How-To guides require judgment**: Don't assume experienced. Ask: "Does this task require prior HSEOS knowledge?" Early-journey tasks (first PRD, first sprint) are newbie docs. Customization and advanced features are experienced.

**If unclear**: Ask the user "Who is the target audience for this document — new users learning HSEOS, or experienced users who know the system?"
...
| `hseos_hsm_*` | `hseos_hsm_workflow-init` | Internal slash command name, platform-specific |
...
- First mention: Run the `help` workflow (`/hseos-help` on most platforms)
...
|----------|----------|-------|
| `workflow-init` | `hseos-help` | DEPRECATED - help system replaces initialization |
| `workflow-status` | `hseos-help` | DEPRECATED - help system replaces status checking |

### The Help System

The `hseos-help` workflow is the modern replacement for both `workflow-init` and `workflow-status`:
...
Users can run `hseos-help` to get guidance on what to do next. It detects:
...
3. **Simple names**: Just the workflow name, no `hseos_hsm_` prefix, no `/` prefix
