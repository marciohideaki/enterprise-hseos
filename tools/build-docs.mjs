/**
 * HSEOS Documentation Build Pipeline
 *
...
const REPO_URL = 'https://github.com/hseos-code-org/HSEOS';
...
// Main Entry Point
/**
 * Orchestrates the full HSEOS documentation build pipeline.
 *
...
async function main() {
  ...
  console.log();
  printBanner('HSEOS Documentation Build Pipeline');
  console.log();
...
function generateLlmsTxt(outputDir) {
  ...
  const content = [
    ...
    '## Quick Start',
    '',
    `- **[Getting Started](${siteUrl}/tutorials/getting-started/)** - Tutorial: install and learn how HSEOS works`,
    `- **[Installation](${siteUrl}/how-to/install-hseos/)** - How to install HSEOS Method`,
    '',
    '## Core Concepts',
    '',
...
function generateLlmsFullTxt(docsDir, outputDir) {
  ...
  const output = [
    '# HSEOS Framework Documentation (Full)',
    '',
