...
class IdeManager {
  ...
  /**
   * Set the hseos folder name for all IDE handlers
  ...
  setHseosFolderName(hseosFolderName) {
    ...
    for (const handler of this.handlers.values()) {
      if (typeof handler.setHseosFolderName === 'function') {
        handler.setHseosFolderName(hseosFolderName);
      }
    }
  }
      ...
      try {
        ...
        if (HandlerClass) {
          ...
          if (instance.name && typeof instance.name === 'string') {
            if (typeof instance.setHseosFolderName === 'function') {
              instance.setHseosFolderName(this.hseosFolderName);
            }
  ...
  async loadConfigDrivenHandlers() {
    ...
    for (const [platformCode, platformInfo] of Object.entries(platformConfig.platforms)) {
      ...
      if (typeof handler.setHseosFolderName === 'function') {
        handler.setHseosFolderName(this.hseosFolderName);
      }
   ...
   */
  async setup(ideName, projectDir, hseosDir, options = {}) {
    ...
    try {
      const handlerResult = await handler.setup(projectDir, hseosDir, options);
