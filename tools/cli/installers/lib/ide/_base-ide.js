...
const { HSEOS_FOLDER_NAME } = require('./shared/path-utils');
...
class BaseIdeSetup {
  ...
  async cleanup(projectDir, options = {}) {
    ...
    if (this.configDir) {
      ...
      if (await fs.pathExists(configPath)) {
        const hseosRulesPath = path.join(configPath, HSEOS_FOLDER_NAME);
        if (await fs.pathExists(hseosRulesPath)) {
          await fs.remove(hseosRulesPath);
          if (!options.silent) await prompts.log.message(`Removed ${this.name} HSEOS configuration`);
        }
      }
    }
  }
  ...
  /**
   * Get list of agents from HSEOS installation
   * @param {string} hseosDir - HSEOS installation directory
   * @returns {Array} List of agent files
   */
  async getAgents(hseosDir) {
    const agents = [];

    // Get core agents
    const coreAgentsPath = path.join(hseosDir, 'core', 'agents');
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_config' && entry.name !== 'agents') {
        const moduleAgentsPath = path.join(hseosDir, entry.name, 'agents');
    ...
    // Get standalone agents from hseos/agents/ directory
    const standaloneAgentsDir = path.join(hseosDir, 'agents');
  ...
  async getTasks(hseosDir, standaloneOnly = false) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_config' && entry.name !== 'agents') {
        const moduleTasksPath = path.join(hseosDir, entry.name, 'tasks');
  ...
  async getTools(hseosDir, standaloneOnly = false) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_config' && entry.name !== 'agents') {
        const moduleToolsPath = path.join(hseosDir, entry.name, 'tools');
  ...
  /**
   * Get list of workflows from HSEOS installation
   * @param {string} hseosDir - HSEOS installation directory
   * @param {boolean} standaloneOnly - If true, only return standalone workflows
   * @returns {Array} List of workflow files
   */
  async getWorkflows(hseosDir, standaloneOnly = false) {
    const workflows = [];

    // Get core workflows
    const coreWorkflowsPath = path.join(hseosDir, 'core', 'workflows');
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_config' && entry.name !== 'agents') {
        const moduleWorkflowsPath = path.join(hseosDir, entry.name, 'workflows');
  ...
  /**
   * Write file with content (replaces _hseos placeholder)
   * @param {string} filePath - File path
   * @param {string} content - File content
   */
  async writeFile(filePath, content) {
    // Replace _hseos placeholder if present
    if (typeof content === 'string' && content.includes('.hseos')) {
      content = content.replaceAll('.hseos', this.hseosFolderName);
    }

    // Replace escape sequence _hseos with literal _hseos
    if (typeof content === 'string' && content.includes('.hseos')) {
      content = content.replaceAll('.hseos', '.hseos');
    }
  ...
  /**
   * Copy file from source to destination (replaces _hseos placeholder in text files)
  ...
  async copyFile(source, dest) {
    ...
    if (textExtensions.includes(ext)) {
      try {
        ...
        // Replace _hseos placeholder with actual folder name
        if (content.includes('.hseos')) {
          content = content.replaceAll('.hseos', this.hseosFolderName);
        }
   ...
   */
  async createAgentConfig(hseosDir, agent) {
    const agentConfigDir = path.join(hseosDir, '_config', 'agents');
