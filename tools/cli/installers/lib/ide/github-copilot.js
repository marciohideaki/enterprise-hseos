...
const { HSEOS_FOLDER_NAME, toDashPath } = require('./shared/path-utils');
...
class GitHubCopilotSetup extends BaseIdeSetup {
  ...
  async setup(projectDir, hseosDir, options = {}) {
    ...
    // Clean up any existing HSEOS files before reinstalling
    await this.cleanup(projectDir);

    // Load agent manifest for enriched descriptions
    const agentManifest = await this.loadAgentManifest(hseosDir);

    // Generate agent launchers
    const agentGen = new AgentCommandGenerator(this.hseosFolderName);
    const { artifacts: agentArtifacts } = await agentGen.collectAgentArtifacts(hseosDir, options.selectedModules || []);
    ...
    // Generate prompt files from hseos-help.csv
    const promptCount = await this.generatePromptFiles(projectDir, hseosDir, agentArtifacts, agentManifest);

    // Generate copilot-instructions.md
    await this.generateCopilotInstructions(projectDir, hseosDir, agentManifest, options);
   ...
   */
  async loadAgentManifest(hseosDir) {
    const manifestPath = path.join(hseosDir, '_config', 'agent-manifest.csv');
  ...
  /**
   * Load hseos-help.csv to drive prompt generation
   * @param {string} hseosDir - HSEOS installation directory
   * @returns {Array|null} Parsed CSV rows
   */
  async loadHseosHelp(hseosDir) {
    const helpPath = path.join(hseosDir, '_config', 'hseos-help.csv');
...
`;
   ...
   */
  async generatePromptFiles(projectDir, hseosDir, agentArtifacts, agentManifest) {
    const promptsDir = path.join(projectDir, this.githubDir, this.promptsDir);
    let promptCount = 0;

    // Load hseos-help.csv to drive workflow/task prompt generation
    const helpEntries = await this.loadHseosHelp(hseosDir);
    ...
    for (const artifact of agentArtifacts) {
      const agentMeta = agentManifest.get(artifact.name);
      const fileName = `hseos-${artifact.name}.prompt.md`;
      const toolsStr = this.getToolsForFile(fileName);
  ...
  /**
   * Create prompt content for a workflow/task entry from hseos-help.csv
   * Determines the pattern (A, B, or A for .xml tasks) based on file extension
   * @param {Object} entry - hseos-help.csv row
  ...
  createWorkflowPromptContent(entry, workflowFile, toolsStr) {
    ...
    if (workflowFile.endsWith('.yaml')) {
      ...
      body = `${configLine}
2. Load the workflow engine at {project-root}/${this.hseosFolderName}/core/tasks/workflow.xml
...
`;
   ...
   * @param {string} name - Entry name from hseos-help.csv
...
`;
   ...
   * @param {Map} agentManifest - Agent manifest data
   */
  async generateCopilotInstructions(projectDir, hseosDir, agentManifest, options = {}) {
    const configVars = await this.loadModuleConfig(hseosDir);
    ...
    const hseos = this.hseosFolderName;
    const hseosSection = `# HSEOS Framework — Project Instructions
...
- **Help manifest**: \`${hseos}/_config/hseos-help.csv\`
- **Agent memory**: \`${hseos}/_memory/\`
...
Type \`/hseos-\` in Copilot Chat to see all available HSEOS workflows and agent activators. Agents are also available in the agents dropdown.`;

    const instructionsPath = path.join(projectDir, this.githubDir, 'copilot-instructions.md');
    const markerStart = '<!-- HSEOS:START -->';
    const markerEnd = '<!-- HSEOS:END -->';
    ...
    if (await fs.pathExists(instructionsPath)) {
      ...
      if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
        // Replace only the HSEOS section between markers
   ...
   */
  async loadModuleConfig(hseosDir) {
  ...
  async collectExistingToolPermissions(projectDir) {
    ...
    const dirs = [
      [path.join(projectDir, this.githubDir, this.agentsDir), /^hseos.*\.agent\.md$/],
      [path.join(projectDir, this.githubDir, this.promptsDir), /^hseos-.*\.prompt\.md$/],
    ];
   ...
   * @param {string} fileName - Target filename (e.g. 'hseos-agent-bmm-pm.agent.md')
  ...
  /**
   * Cleanup GitHub Copilot configuration - surgically remove only HSEOS files
   */
  async cleanup(projectDir, options = {}) {
    ...
    if (await fs.pathExists(agentsDir)) {
      ...
      for (const file of files) {
        if (file.startsWith('hseos') && (file.endsWith('.agent.md') || file.endsWith('.md'))) {
      ...
      if (removed > 0 && !options.silent) {
        await prompts.log.message(`  Cleaned up ${removed} existing HSEOS agents`);
      }
    }
    ...
    if (await fs.pathExists(promptsDir)) {
      ...
      for (const file of files) {
        if (file.startsWith('hseos-') && file.endsWith('.prompt.md')) {
      ...
      if (removed > 0 && !options.silent) {
        await prompts.log.message(`  Cleaned up ${removed} existing HSEOS prompts`);
      }
    }

    // During uninstall, also strip HSEOS markers from copilot-instructions.md.
  ...
  /**
   * Strip HSEOS marker section from copilot-instructions.md
