 ...
 *
 * This class provides a standardized way to install HSEOS artifacts to IDEs
...
class ConfigDrivenIdeSetup extends BaseIdeSetup {
  ...
  async setup(projectDir, hseosDir, options = {}) {
    // Check for HSEOS files in ancestor directories that would cause duplicates
    if (this.installerConfig?.ancestor_conflict_check) {
      const conflict = await this.findAncestorConflict(projectDir);
      if (conflict) {
        await prompts.log.error(
          `Found existing HSEOS commands in ancestor installation: ${conflict}\n` +
            `  ${this.name} inherits commands from parent directories, so this would cause duplicates.\n` +
            `  Please remove the HSEOS files from that directory first:\n` +
            `    rm -rf "${conflict}"/hseos*`,
        );
        return {
          success: false,
    ...
    if (this.installerConfig.targets) {
      return this.installToMultipleTargets(projectDir, hseosDir, this.installerConfig.targets, options);
    }

    // Handle single-target installations
    if (this.installerConfig.target_dir) {
      return this.installToTarget(projectDir, hseosDir, this.installerConfig, options);
    }
   ...
   */
  async installToTarget(projectDir, hseosDir, config, options) {
    ...
    if (!artifact_types || artifact_types.includes('agents')) {
      ...
      const { artifacts } = await agentGen.collectAgentArtifacts(hseosDir, selectedModules);
    ...
    if (!artifact_types || artifact_types.includes('workflows')) {
      ...
      const { artifacts } = await workflowGen.collectWorkflowArtifacts(hseosDir);
    ...
    if (!artifact_types || artifact_types.includes('tasks') || artifact_types.includes('tools')) {
      ...
      const { artifacts } = await taskToolGen.collectTaskToolArtifacts(hseosDir);
   ...
   * @returns {Promise<Object>} Installation result
   */
  async installToMultipleTargets(projectDir, hseosDir, targets, options) {
    const allResults = { agents: 0, workflows: 0, tasks: 0, tools: 0 };

    for (const target of targets) {
      const result = await this.installToTarget(projectDir, hseosDir, target, options);
      if (result.success) {
...
1. LOAD the FULL agent file from {project-root}/{{hseosFolderName}}/{{path}}
...
LOAD and execute from: {project-root}/{{hseosFolderName}}/{{path}}
`;
  }
  ...
  renderTemplate(template, artifact) {
    ...
    rendered = rendered.replaceAll('.hseos', this.hseosFolderName);

    // Replace {{hseosFolderName}} placeholder if present
    rendered = rendered.replaceAll('{{hseosFolderName}}', this.hseosFolderName);
  ...
  generateFilename(artifact, artifactType, extension = '.md') {
    ...
    // If using default markdown, preserve the hseos-agent- prefix for agents
    if (extension === '.md') {
      return baseName;
    }
  ...
  async cleanupTarget(projectDir, targetDir, options = {}) {
    ...
    if (removedCount > 0 && !options.silent) {
      await prompts.log.message(`  Cleaned ${removedCount} HSEOS files from ${targetDir}`);
    }
  ...
  /**
   * Check ancestor directories for existing HSEOS files in the same target_dir.
    ...
    while (current !== root && current.length > root.length) {
      ...
      try {
        if (await fs.pathExists(candidatePath)) {
          ...
          const hasHseos = entries.some((e) => typeof e === 'string' && e.toLowerCase().startsWith('hseos'));
          if (hasHseos) {
