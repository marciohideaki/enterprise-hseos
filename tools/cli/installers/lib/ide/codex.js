...
const { getTasksFromHseos } = require('./shared/hseos-artifacts');
...
class CodexSetup extends BaseIdeSetup {
  ...
  async setup(projectDir, hseosDir, options = {}) {
    ...
    const tasks = await getTasksFromHseos(hseosDir, options.selectedModules || []);
    const taskArtifacts = [];
    ...
    const ttGen = new TaskToolCommandGenerator(this.hseosFolderName);
    ...
    const workflowGenerator = new WorkflowCommandGenerator(this.hseosFolderName);
    const { artifacts: workflowArtifacts } = await workflowGenerator.collectWorkflowArtifacts(hseosDir);
  ...
  /**
   * Detect Codex installation by checking for HSEOS skills
   */
  async detect(projectDir) {
    ...
    if (await fs.pathExists(dir)) {
      try {
        ...
        if (entries && entries.some((entry) => entry && typeof entry === 'string' && entry.startsWith('hseos'))) {
          return true;
        }
      } catch {
        // Ignore errors
   ...
   */
  async collectClaudeArtifacts(projectDir, hseosDir, options = {}) {
    ...
    const tasks = await getTasksFromHseos(hseosDir, selectedModules);
    for (const task of tasks) {
      const content = await this.readAndProcessWithProject(
        task.path,
        {
    ...
    const workflowGenerator = new WorkflowCommandGenerator(this.hseosFolderName);
    const { artifacts: workflowArtifacts, counts: workflowCounts } = await workflowGenerator.collectWorkflowArtifacts(hseosDir);
  ...
  async writeSkillArtifacts(destDir, artifacts, artifactType) {
    ...
    for (const artifact of artifacts) {
      ...
      // Get the dash-format name (e.g., hseos-bmm-create-prd.md) and remove .md
  ...
  /**
   * Remove existing HSEOS skill directories from the skills directory.
   */
  async clearOldHseosSkills(destDir, options = {}) {
  ...
  /**
   * Clean old HSEOS files from legacy .codex/prompts directories.
   */
  async clearOldHseosFiles(destDir, options = {}) {
    if (!(await fs.pathExists(destDir))) {
      return;
    }
  ...
  async cleanup(projectDir = null) {
    ...
    const oldGlobalDir = this.getOldCodexPromptDir(null, 'global');
    await this.clearOldHseosFiles(oldGlobalDir);

    if (projectDir) {
      const oldProjectDir = this.getOldCodexPromptDir(projectDir, 'project');
      await this.clearOldHseosFiles(oldProjectDir);

      // Clean new .agents/skills location
      const destDir = this.getCodexSkillsDir(projectDir);
      await this.clearOldHseosSkills(destDir);
    }
  }
