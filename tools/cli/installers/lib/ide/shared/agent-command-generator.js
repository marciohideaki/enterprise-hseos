...
class AgentCommandGenerator {
   ...
   */
  async collectAgentArtifacts(hseosDir, selectedModules = []) {
    const { getAgentsFromHseos } = require('./hseos-artifacts');

    // Get agents from INSTALLED hseos/ directory
    const agents = await getAgentsFromHseos(hseosDir, selectedModules);
    ...
    for (const agent of agents) {
      ...
      if (agentRelPath.includes('_hseos/')) {
        const parts = agentRelPath.split(/_hseos\//);
        if (parts.length > 1) {
   ...
   *
   * The hseos-agent- prefix distinguishes agents from workflows/tasks/tools.
   *
   * @param {string} baseCommandsDir - Base commands directory for the IDE
   * @param {Array} artifacts - Agent launcher artifacts
   * @returns {number} Count of launchers written
  ...
  async writeDashArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'agent-launcher') {
        // Convert relativePath to dash format: bmm/agents/pm.md → hseos-agent-bmm-pm.md
