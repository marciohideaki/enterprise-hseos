...
/**
 * Helpers for gathering HSEOS agents/tasks from the installed tree.
 * Shared by installers that need Claude-style exports.
 */
async function getAgentsFromHseos(hseosDir, selectedModules = []) {
  const agents = [];

  // Get core agents
  if (await fs.pathExists(path.join(hseosDir, 'core', 'agents'))) {
    const coreAgents = await getAgentsFromDir(path.join(hseosDir, 'core', 'agents'), 'core');
    agents.push(...coreAgents);
  }

  // Get module agents
  for (const moduleName of selectedModules) {
    const agentsPath = path.join(hseosDir, moduleName, 'agents');
...
async function getTasksFromHseos(hseosDir, selectedModules = []) {
  const tasks = [];

  if (await fs.pathExists(path.join(hseosDir, 'core', 'tasks'))) {
    const coreTasks = await getTasksFromDir(path.join(hseosDir, 'core', 'tasks'), 'core');
    tasks.push(...coreTasks);
  }

  for (const moduleName of selectedModules) {
    const tasksPath = path.join(hseosDir, moduleName, 'tasks');
