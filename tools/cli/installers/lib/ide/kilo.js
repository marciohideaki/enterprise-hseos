...
class KiloSetup extends BaseIdeSetup {
  ...
  async setup(projectDir, hseosDir, options = {}) {
    ...
    // Load existing config (may contain non-HSEOS modes and other settings)
    ...
    // Clear old HSEOS workflows before writing new ones
    await this.clearHseosWorkflows(workflowsDir);
  ...
  async createModeObject(artifact, projectDir) {
    ...
    return {
      slug: `hseos-${artifact.module}-${artifact.name}`,
      name: `${icon} ${title}`,
  ...
  /**
   * Clear old HSEOS workflow files from workflows directory
   * @param {string} workflowsDir - Workflows directory path
   */
  async clearHseosWorkflows(workflowsDir) {
    const fs = require('fs-extra');
    if (!(await fs.pathExists(workflowsDir))) return;

    const entries = await fs.readdir(workflowsDir);
    for (const entry of entries) {
      if (entry.startsWith('hseos-') && entry.endsWith('.md')) {
    ...
    if (await fs.pathExists(kiloModesPath)) {
      ...
      try {
        ...
        if (Array.isArray(config.customModes)) {
          ...
          // Remove HSEOS modes only (keep non-HSEOS modes)
          config.customModes = config.customModes.filter((mode) => !mode.slug || !mode.slug.startsWith('hseos-'));
          ...
          if (removedCount > 0) {
            ...
            if (!options.silent) await prompts.log.message(`Removed ${removedCount} HSEOS modes from .kilocodemodes`);
          }
        }
      } catch {
  ...
  async installCustomAgentLauncher(projectDir, agentName, agentPath, metadata) {
    ...
    const slug = `hseos-custom-${agentName.toLowerCase()}`;
    ...
    config.customModes.push({
      slug: slug,
      name: `HSEOS Custom: ${agentName}`,
      description: `Custom HSEOS agent: ${agentName}\n\n**⚠️ IMPORTANT**: Run @${agentPath} first to load the complete agent!\n\nThis is a launcher for the custom HSEOS agent "${agentName}". The agent will follow the persona and instructions from the main agent file.\n`,
      prompt: `@${agentPath}\n`,
      always: false,
      permissions: 'all',
    });
