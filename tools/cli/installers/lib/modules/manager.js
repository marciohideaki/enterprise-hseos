...
/**
 * Manages the installation, updating, and removal of HSEOS modules.
 ...
 * await manager.install('core-module', '/path/to/hseos');
 */
class ModuleManager {
  constructor(options = {}) {
    this.xmlHandler = new XmlHandler();
    this.hseosFolderName = HSEOS_FOLDER_NAME; // Default, can be overridden
    this.customModulePaths = new Map(); // Initialize custom module paths
    this.externalModuleManager = new ExternalModuleManager(); // For external official modules
  }

  /**
   * Set the hseos folder name for placeholder replacement
   * @param {string} hseosFolderName - The hseos folder name
   */
  setHseosFolderName(hseosFolderName) {
  ...
  /**
   * Copy sidecar directory to .hseos/_memory location with update-safe handling
   * @param {string} sourceSidecarPath - Source sidecar directory path
   * @param {string} agentName - Name of the agent (for naming)
   * @param {string} hseosMemoryPath - This should ALWAYS be .hseos/_memory
   ...
   */
  async copySidecarToMemory(sourceSidecarPath, agentName, hseosMemoryPath, isUpdate = false, hseosDir = null, installer = null) {
    const crypto = require('node:crypto');
    const sidecarTargetDir = path.join(hseosMemoryPath, `${agentName}-sidecar`);

    // Ensure target directory exists
    await fs.ensureDir(hseosMemoryPath);
    ...
    if (isUpdate && installer) {
      existingFilesManifest = await installer.readFilesManifest(hseosDir);
    }
    ...
    for (const file of sourceFiles) {
      ...
      // Path relative to hseos directory
      const relativeToHseos = path.join('_memory', `${agentName}-sidecar`, file);
      ...
      if (isUpdate && (await fs.pathExists(targetFilePath))) {
        ...
        const lastKnownHash = existingSidecarFiles.get(relativeToHseos);
        ...
        if (lastKnownHash) {
          ...
          if (currentTargetHash === lastKnownHash) {
            ...
            if (process.env.HSEOS_VERBOSE_INSTALL === 'true') {
              await prompts.log.message(`    Updated sidecar file: ${relativeToHseos}`);
            }
          } else {
            // User has modified the file, preserve it
            if (process.env.HSEOS_VERBOSE_INSTALL === 'true') {
              await prompts.log.message(`    Preserving user-modified file: ${relativeToHseos}`);
            }
          }
        } else {
          ...
          await this.copyFileWithPlaceholderReplacement(sourceFilePath, targetFilePath, true);
          if (process.env.HSEOS_VERBOSE_INSTALL === 'true') {
            await prompts.log.message(`    Added new sidecar file: ${relativeToHseos}`);
          }
        }
      } else {
        // New installation
        await this.copyFileWithPlaceholderReplacement(sourceFilePath, targetFilePath, true);
        if (process.env.HSEOS_VERBOSE_INSTALL === 'true') {
          await prompts.log.message(`    Copied sidecar file: ${relativeToHseos}`);
        }
      }
   ...
   * bmm is the only built-in module, directly under src/hsm
  ...
  async listAvailable() {
    ...
    // Add built-in hsm module (directly under src/hsm)
    const bmmPath = getSourcePath('hsm');
    if (await fs.pathExists(bmmPath)) {
      const bmmInfo = await this.getModuleInfo(bmmPath, 'hsm', 'src/hsm');
      if (bmmInfo) {
    ...
    if (this.hseosDir) {
      const customCacheDir = path.join(this.hseosDir, '_config', 'custom');
  ...
  async getModuleInfo(modulePath, defaultName, sourceDescription) {
    ...
    // Mark as custom if it's using custom.yaml OR if it's outside src/hsm or src/core
    const isCustomSource = sourceDescription !== 'src/hsm' && sourceDescription !== 'src/core' && sourceDescription !== 'src/modules';
    const moduleInfo = {
      id: defaultName,
      path: modulePath,
      name: defaultName
  ...
  async findModuleSource(moduleCode, options = {}) {
    ...
    // Check for built-in bmm module (directly under src/hsm)
    if (moduleCode === 'hsm') {
      const bmmPath = getSourcePath('hsm');
      if (await fs.pathExists(bmmPath)) {
        return bmmPath;
  ...
  getExternalCacheDir() {
    ...
    const cacheDir = path.join(os.homedir(), '.hseos', 'cache', 'external-modules');
    return cacheDir;
  }
   ...
   * @param {string} hseosDir - Target hseos directory
   ...
   */
  async install(moduleName, hseosDir, fileTrackingCallback = null, options = {}) {
    const sourcePath = await this.findModuleSource(moduleName, { silent: options.silent });
    const targetPath = path.join(hseosDir, moduleName);
    ...
    await this.compileModuleAgents(sourcePath, targetPath, moduleName, hseosDir, options.installer);
    ...
    if (!options.skipModuleInstaller) {
      await this.createModuleDirectories(moduleName, hseosDir, options);
    }

    // Capture version info for manifest
    const { Manifest } = require('../core/manifest');
    const manifestObj = new Manifest();
    const versionInfo = await manifestObj.getModuleVersionInfo(moduleName, hseosDir, sourcePath);

    await manifestObj.addModule(hseosDir, moduleName, {
   ...
   */
  async update(moduleName, hseosDir, force = false, options = {}) {
    ...
    if (force) {
      ...
      await fs.remove(targetPath);
      return await this.install(moduleName, hseosDir, null, { installer: options.installer });
    } else {
   ...
   * @param {string} hseosDir - Target hseos directory
   */
  async remove(moduleName, hseosDir) {
    const targetPath = path.join(hseosDir, moduleName);
   ...
   */
  async isInstalled(moduleName, hseosDir) {
    const targetPath = path.join(hseosDir, moduleName);
   ...
   */
  async getInstalledInfo(moduleName, hseosDir) {
    const targetPath = path.join(hseosDir, moduleName);
  ...
  async copyWorkflowYamlStripped(sourceFile, targetFile) {
    ...
    yamlContent = yamlContent.replaceAll('.hseos', this.hseosFolderName);
   ...
   * @param {Object} installer - Installer instance for file tracking
   */
  async compileModuleAgents(sourcePath, targetPath, moduleName, hseosDir, installer = null) {
      ...
      try {
        ...
        if (!(await fs.pathExists(customizePath))) {
          ...
          if (await fs.pathExists(genericTemplatePath)) {
            ...
            manifestData.agentCustomizations[path.relative(hseosDir, customizePath)] = originalHash;
        ...
        if (hasSidecar) {
          ...
          if (await fs.pathExists(sourceSidecarPath)) {
            ...
            const hseosMemoryPath = path.join(hseosDir, '_memory');
          ...
          } else if (process.env.HSEOS_VERBOSE_INSTALL === 'true') {
  ...
  async vendorCrossModuleWorkflows(sourcePath, targetPath, moduleName) {
    ...
    for (const agentFile of yamlFiles) {
      ...
      for (const item of workflowInstallItems) {
        ...
        // Handle both .hseos placeholder and hardcoded folder name
        // Example: {project-root}/.hseos/hsm/workflows/4-implementation/create-story/workflow.yaml
        // Or: {project-root}/hseos/hsm/workflows/4-implementation/create-story/workflow.yaml
        const sourceMatch = sourceWorkflowPath.match(/\{project-root\}\/(?:\.hseos)\/([^/]+)\/workflows\/(.+)/);
        if (!sourceMatch) {
        ...
        // Handle .hseos
        // Example: {project-root}/.hseos/bmgd/workflows/4-production/create-story/workflow.yaml
        const installMatch = installWorkflowPath.match(/\{project-root\}\/(\.hseos)\/([^/]+)\/workflows\/(.+)/);
        if (!installMatch) {
  ...
  async updateWorkflowConfigSource(workflowYamlPath, newModuleName) {
    ...
    // Replace config_source: "{project-root}/.hseos/OLD_MODULE/config.yaml"
    // with config_source: "{project-root}/.hseos/NEW_MODULE/config.yaml"
    // Note: At this point .hseos has already been replaced with actual folder name
    const configSourcePattern = /config_source:\s*["']?\{project-root\}\/[^/]+\/[^/]+\/config\.yaml["']?/g;
    const newConfigSource = `config_source: "{project-root}/${this.hseosFolderName}/${newModuleName}/config.yaml"`;

    const updatedYaml = yamlContent.replaceAll(configSourcePattern, newConfigSource);

    if (updatedYaml !== yamlContent) {
      await fs.writeFile(workflowYamlPath, updatedYaml, 'utf8');
      await prompts.log.message(`      Updated config_source to: ${this.hseosFolderName}/${newModuleName}/config.yaml`);
    }
  }
   ...
   */
  async createModuleDirectories(moduleName, hseosDir, options = {}) {
  ...
  async processModuleConfig(modulePath, moduleName) {
    ...
    if (await fs.pathExists(configPath)) {
      try {
        ...
        configContent = configContent.replaceAll('{project-root}', `hseos/${moduleName}`);
