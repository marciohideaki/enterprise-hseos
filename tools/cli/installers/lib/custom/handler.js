...
class CustomHandler {
   ...
   */
  async install(customPath, hseosDir, config, fileTrackingCallback = null) {
    const results = {
      agentsInstalled: 0,
    ...
    try {
      // Create custom directories in hseos
      const hseosCustomDir = path.join(hseosDir, 'custom');
      const hseosAgentsDir = path.join(hseosCustomDir, 'agents');
      const hseosWorkflowsDir = path.join(hseosCustomDir, 'workflows');

      await fs.ensureDir(hseosCustomDir);
      await fs.ensureDir(hseosAgentsDir);
      await fs.ensureDir(hseosWorkflowsDir);

      // Process agents - compile and copy agents
      const agentsDir = path.join(customPath, 'agents');
      if (await fs.pathExists(agentsDir)) {
        await this.compileAndCopyAgents(agentsDir, hseosAgentsDir, hseosDir, config, fileTrackingCallback, results);
      ...
      if (await fs.pathExists(workflowsDir)) {
        await this.copyDirectory(workflowsDir, hseosWorkflowsDir, results, fileTrackingCallback, config);
      ...
      for (const entry of entries) {
        if (entry.isFile() && entry.name !== 'custom.yaml' && !entry.name.startsWith('.') && !entry.name.endsWith('.md')) {
          ...
          const targetPath = path.join(hseosCustomDir, entry.name);
   ...
   * @param {Object} results - Results object to update
   */
  async compileAndCopyAgents(sourceAgentsPath, targetAgentsPath, hseosDir, config, fileTrackingCallback, results) {
    ...
    for (const agentFile of agentFiles) {
      ...
      // Use the actual hseosDir if available (for when installing to temp dir)
      const actualHseosDir = config._hseosDir || hseosDir;
      const customizePath = path.join(actualHseosDir, '_config', 'agents', `custom-${agentName}.customize.yaml`);
