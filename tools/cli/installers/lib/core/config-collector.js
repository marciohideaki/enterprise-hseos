...
class ConfigCollector {
   ...
   * @returns {Promise<string>} Path to hseos directory
   */
  async findHseosDir(projectDir) {
    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      // Project doesn't exist yet, return default
      return path.join(projectDir, 'hseos');
    }
  ...
  /**
   * Detect the existing HSEOS folder name in a project
   * @param {string} projectDir - Project directory
   * @returns {Promise<string|null>} Folder name (just the name, not full path) or null if not found
   */
  async detectExistingHseosFolder(projectDir) {
    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      return null;
    }
  ...
  async loadExistingConfig(projectDir) {
    ...
    // Find the actual hseos directory (handles custom folder names)
    ...
    // Dynamically discover all installed modules by scanning hseos directory
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const moduleConfigPath = path.join(hseosDir, entry.name, 'config.yaml');
