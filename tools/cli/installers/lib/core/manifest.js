...
class Manifest {
   ...
   */
  async create(hseosDir, data, installedFiles = []) {
    ...
    // Get the HSEOS version from package.json
    const hseosVersion = data.version || require(path.join(process.cwd(), 'package.json')).version;

    // Convert module list to new detailed format
    const moduleDetails = [];
    if (data.modules && Array.isArray(data.modules)) {
      for (const moduleName of data.modules) {
        // Core and BMM modules use the HSEOS version
        const moduleVersion = moduleName === 'core' || moduleName === 'hsm' ? hseosVersion : null;
        ...
        moduleDetails.push({
          ...
          installDate: now,
          lastUpdated: now,
          source: moduleName === 'core' || moduleName === 'hsm' ? 'built-in' : 'unknown',
        });
      }
    }
   ...
   */
  async read(hseosDir) {
    const yamlPath = path.join(hseosDir, '_config', 'manifest.yaml');
    const yaml = require('yaml');
   ...
   */
  async update(hseosDir, updates, installedFiles = null) {
    const yaml = require('yaml');
    const manifest = (await this._readRaw(hseosDir)) || {
      installation: {},
      modules: [],
      ides: [],
    };
   ...
   */
  async addModule(hseosDir, moduleName, options = {}) {
    const manifest = await this._readRaw(hseosDir);
    if (!manifest) {
    ...
    await this._writeRaw(hseosDir, manifest);
  }
   ...
   */
  async removeModule(hseosDir, moduleName) {
    const manifest = await this._readRaw(hseosDir);
   ...
   */
  async updateModuleVersion(hseosDir, moduleName, versionInfo) {
    const manifest = await this._readRaw(hseosDir);
    if (!manifest || !manifest.modules) {
      return;
    }
   ...
   */
  async getModuleVersion(hseosDir, moduleName) {
    const manifest = await this._readRaw(hseosDir);
    if (!manifest || !manifest.modules) {
      return null;
    }
   ...
   */
  async getAllModuleVersions(hseosDir) {
    const manifest = await this._readRaw(hseosDir);
    if (!manifest || !manifest.modules) {
      return [];
    }
   ...
   */
  async _writeRaw(hseosDir, manifestData) {
    const yaml = require('yaml');
    const manifestPath = path.join(hseosDir, '_config', 'manifest.yaml');
   ...
   */
  async addIde(hseosDir, ideName) {
    const manifest = await this.read(hseosDir);
    if (!manifest) {
      throw new Error('No manifest found');
    }
    ...
    if (!manifest.ides.includes(ideName)) {
      ...
      await this.update(hseosDir, { ides: manifest.ides });
    }
  }
   ...
   * @param {string} hseosDir - Path to hseos directory for relative paths
   * @returns {Array} Array of file metadata objects
   */
  async parseInstalledFiles(installedFiles, hseosDir) {
    const fileMetadata = [];

    for (const filePath of installedFiles) {
      const fileExt = path.extname(filePath).toLowerCase();
      // Make path relative to parent of hseos directory, starting with 'hseos/'
      const relativePath = 'hseos' + filePath.replace(hseosDir, '').replaceAll('\\', '/');
   ...
   * @param {string} relativePath - Relative path starting with 'hseos/'
   ...
   */
  async addCustomModule(hseosDir, customModule) {
    const manifest = await this.read(hseosDir);
    if (!manifest) {
      throw new Error('No manifest found');
    }
    ...
    await this.update(hseosDir, { customModules: manifest.customModules });
  }
   ...
   */
  async removeCustomModule(hseosDir, moduleId) {
    const manifest = await this.read(hseosDir);
   ...
   */
  async getModuleVersionInfo(moduleName, hseosDir, moduleSourcePath = null) {
    const os = require('node:os');
    const yaml = require('yaml');

    // Built-in modules use HSEOS version (only core and bmm are in HSEOS repo)
    if (['core', 'hsm'].includes(moduleName)) {
      const hseosVersion = require(path.join(getProjectRoot(), 'package.json')).version;
      return {
        version: hseosVersion,
        source: 'built-in',
        npmPackage: null,
    ...
    if (moduleInfo) {
      ...
      if (!version) {
        const cacheDir = path.join(os.homedir(), '.hseos', 'cache', 'external-modules', moduleName);
    ...
    const cacheDir = path.join(hseosDir, '_config', 'custom', moduleName);
   ...
   */
  async checkForUpdates(hseosDir) {
    const modules = await this.getAllModuleVersions(hseosDir);
    const updates = [];
