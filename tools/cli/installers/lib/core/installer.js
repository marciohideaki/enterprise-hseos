...
`;
  ...
  async writeStateManagementConfig(projectDir, stateManagement) {
    await this.writeProjectConfigSection(projectDir, 'state_management', stateManagement);
  }
  ...
  async writeSecondBrainConfig(projectDir, secondBrain) {
    await this.writeProjectConfigSection(projectDir, 'second_brain', secondBrain);
  }

  /**
   * Create or update one section of the project-scoped HSEOS configuration.
   * Fresh headless installs do not have a platform adapter that can create the
   * file first, so section writers own the bootstrap boundary.
   * @param {string} projectDir - Project directory
   * @param {string} section - Top-level YAML key
   * @param {Object} value - Section value
   */
  async writeProjectConfigSection(projectDir, section, value) {
    const yaml = require('yaml');
    const configPath = path.join(projectDir, '.hseos', 'config', 'hseos.config.yaml');
    await fs.ensureDir(path.dirname(configPath));

    const doc = (await fs.pathExists(configPath))
      ? yaml.parseDocument(await fs.readFile(configPath, 'utf8'))
      : yaml.parseDocument('# HSEOS Project Configuration\n# Generated by `hseos install`\n');
    if (doc.errors.length > 0) {
      throw new Error(`Cannot update invalid project configuration: ${doc.errors[0].message}`);
    }
    doc.set(section, doc.createNode(value));
...
`;
  ...
  async writeUsageDashboardConfig(projectDir, dashboardConfig) {
    await this.writeProjectConfigSection(projectDir, 'usageDashboard', dashboardConfig);
  }
  ...
  async writeRtkConfig(projectDir, rtkConfig) {
    await this.writeProjectConfigSection(projectDir, 'rtk', rtkConfig);
  }
