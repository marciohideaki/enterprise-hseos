...
class AgentCoreCompiler {
  ...
  async compile(projectDir, hseosDir, options = {}) {
    ...
    const hooks = await this.writeHookRegistry(root, sourceHooksPath);
    await this.writePlatformAdapters(root, sourceHooksPath, options.platforms || []);
...
`;
  ...
  async writeHookRegistry(root, claudeHooksPath) {
  ...
  async writePlatformAdapters(root, sourceHooksPath, platforms) {
    if (!platforms.includes('claude-code') || !sourceHooksPath || !(await fs.pathExists(sourceHooksPath))) {
      return;
    }

    const targetHooksPath = path.join(root, '.claude', 'hooks.json');
    await fs.ensureDir(path.dirname(targetHooksPath));
    await fs.copy(sourceHooksPath, targetHooksPath, { overwrite: true });
  }
