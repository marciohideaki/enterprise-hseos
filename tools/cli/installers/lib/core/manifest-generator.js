...
class ManifestGenerator {
   ...
   */
  async generateManifests(hseosDir, selectedModules, installedFiles = [], options = {}) {
    ...
    // Scan the hseos directory to find all actually installed modules
    const installedModules = await this.scanInstalledModules(hseosDir);
    ...
    this.hseosDir = hseosDir;
    this.hseosFolderName = path.basename(hseosDir); // Get the actual folder name (e.g., '.hseos' or 'hseos')
   ...
   * Scans the INSTALLED hseos directory, not the source
   */
  async collectWorkflows(selectedModules) {
    ...
    for (const moduleName of this.updatedModules) {
      const modulePath = path.join(this.hseosDir, moduleName);
  ...
  async getWorkflowsFromPath(basePath, moduleName) {
    ...
    const debug = process.env.HSEOS_DEBUG_MANIFEST === 'true';
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (workflow.name && workflow.description) {
              ...
              const installPath =
                moduleName === 'core'
                  ? `${this.hseosFolderName}/core/workflows/${relativePath}/${entry.name}`
                  : `${this.hseosFolderName}/${moduleName}/workflows/${relativePath}/${entry.name}`;
  ...
  async collectAgents(selectedModules) {
    ...
    // Use updatedModules which already includes deduplicated 'core' + selectedModules
    for (const moduleName of this.updatedModules) {
      const agentsPath = path.join(this.hseosDir, moduleName, 'agents');
    ...
    const standaloneAgentsDir = path.join(this.hseosDir, 'agents');
  ...
  async getAgentsFromDir(dirPath, moduleName, relativePath = '') {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const installPath =
          moduleName === 'core'
            ? `${this.hseosFolderName}/core/agents/${fileRelativePath}`
            : `${this.hseosFolderName}/${moduleName}/agents/${fileRelativePath}`;
  ...
  async collectTasks(selectedModules) {
    ...
    // Use updatedModules which already includes deduplicated 'core' + selectedModules
    for (const moduleName of this.updatedModules) {
      const tasksPath = path.join(this.hseosDir, moduleName, 'tasks');
  ...
  async getTasksFromDir(dirPath, moduleName) {
    ...
    for (const file of files) {
      ...
      if (file.endsWith('.xml') || file.endsWith('.md')) {
        ...
        // Build relative path for installation
        const installPath =
          moduleName === 'core' ? `${this.hseosFolderName}/core/tasks/${file}` : `${this.hseosFolderName}/${moduleName}/tasks/${file}`;
  ...
  async collectTools(selectedModules) {
    ...
    // Use updatedModules which already includes deduplicated 'core' + selectedModules
    for (const moduleName of this.updatedModules) {
      const toolsPath = path.join(this.hseosDir, moduleName, 'tools');
  ...
  async getToolsFromDir(dirPath, moduleName) {
    ...
    for (const file of files) {
      ...
      if (file.endsWith('.xml') || file.endsWith('.md')) {
        ...
        // Build relative path for installation
        const installPath =
          moduleName === 'core' ? `${this.hseosFolderName}/core/tools/${file}` : `${this.hseosFolderName}/${moduleName}/tools/${file}`;
  ...
  async writeMainManifest(cfgDir) {
    ...
    for (const moduleName of this.modules) {
      ...
      const versionInfo = await manifestObj.getModuleVersionInfo(moduleName, this.hseosDir);
  ...
  async writeFilesManifest(cfgDir) {
    ...
    if (this.allInstalledFiles && this.allInstalledFiles.length > 0) {
      ...
      for (const filePath of this.allInstalledFiles) {
        // Store paths relative to hseosDir (no folder prefix)
        const relativePath = filePath.replace(this.hseosDir, '').replaceAll('\\', '/').replace(/^\//, '');
      ...
      for (const file of this.files) {
        ...
        const relPath = file.path.replace(this.hseosFolderName + '/', '');
        const filePath = path.join(this.hseosDir, relPath);
        const hash = await this.calculateFileHash(filePath);
        allFiles.push({
          ...file,
          path: relPath,
  ...
  /**
   * Scan the hseos directory to find all installed modules
   * @param {string} hseosDir - Path to hseos directory
   * @returns {Array} List of module names
   */
  async scanInstalledModules(hseosDir) {
    const modules = [];
