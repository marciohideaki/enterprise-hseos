...
/**
 * Dependency Resolver for HSEOS modules
...
class DependencyResolver {
   ...
   */
  async resolve(hseosDir, selectedModules = [], options = {}) {
    if (options.verbose) {
    ...
    const primaryFiles = await this.collectPrimaryFiles(hseosDir, modulesToProcess, options);
    ...
    const resolvedDeps = await this.resolveDependencyPaths(hseosDir, allDependencies);

    // Fourth pass: check for transitive dependencies
    const transitiveDeps = await this.resolveTransitiveDependencies(hseosDir, resolvedDeps);

    // Combine all files
    const allFiles = new Set([...primaryFiles.map((f) => f.path), ...resolvedDeps, ...transitiveDeps]);

    // Organize by module
    const organizedFiles = this.organizeByModule(hseosDir, allFiles);
   ...
   */
  async collectPrimaryFiles(hseosDir, modules, options = {}) {
    const files = [];
  ...
  async parseDependencies(files) {
    ...
    for (const file of files) {
      ...
      for (const ref of fileRefs) {
        ...
        // Paths starting with hseos/ are absolute references to the hseos installation
        const depType = ref.startsWith('hseos/') ? 'hseos-path' : 'file';
        allDeps.add({
          from: file.path,
          dependency: ref,
          type: depType,
  ...
  parseCommandReferences(content) {
    ...
    const commandPattern = /@(task-|agent-|hseos-)([a-z0-9-]+)/g;
    let match;
    ...
    // Match file paths like hseos/core/agents/analyst
    const pathPattern = /hseos\/(core|bmm|cis)\/(agents|tasks)\/([a-z0-9-]+)/g;
  ...
  parseFileReferences(content) {
    ...
    while ((match = execPattern.exec(content)) !== null) {
      ...
      if (execPath && execPath !== '*') {
        ...
        // Usage is like {project-root}/hseos/core/tasks/foo.md
   ...
   */
  async resolveDependencyPaths(hseosDir, dependencies) {
    const resolved = new Set();

    for (const dep of dependencies) {
      const resolvedPaths = await this.resolveSingleDependency(hseosDir, dep);
   ...
   */
  async resolveSingleDependency(hseosDir, dep) {
    const paths = [];
    ...
    switch (dep.type) {
      ...
      case 'file': {
        ...
        if (depPath.includes('{project-root}')) {
          // Remove {project-root} and resolve as hseos path
          depPath = depPath.replace('{project-root}', '');

          if (depPath.startsWith('hseos/')) {
            const hsPath = depPath.replace(/^hseos\//, '');

            // Handle glob patterns
            if (depPath.includes('*')) {
              // Extract the base path and pattern
              const pathParts = hsPath.split('/');
              ...
              let basePath;
              if (module === 'core') {
                basePath = path.join(hseosDir, 'core', middlePath);
              } else {
                basePath = path.join(hseosDir, 'modules', module, middlePath);
              }
              ...
              // Direct path
              if (hsPath.startsWith('core/')) {
                const corePath = path.join(hseosDir, hsPath);
                if (await fs.pathExists(corePath)) {
                  paths.push(corePath);
                }
              } else {
                const parts = hsPath.split('/');
                const module = parts[0];
                const rest = parts.slice(1).join('/');
                const modulePath = path.join(hseosDir, 'modules', module, rest);
      ...
      case 'command': {
        ...
        const commandPath = await this.resolveCommandToPath(hseosDir, dep.dependency);
        if (commandPath) {
      ...
      case 'hseos-path': {
        // Resolve hseos/ paths (from {project-root}/hseos/... references)
        // These are paths relative to the src directory structure
        const hsPath = dep.dependency.replace(/^hseos\//, '');

        // Try to resolve as if it's in src structure
        // hseos/core/tasks/foo.md -> src/core/tasks/foo.md
        // hseos/hsm/tasks/bar.md -> src/bmm/tasks/bar.md (bmm is directly under src/)
        // hseos/cis/agents/bar.md -> src/modules/cis/agents/bar.md
        ...
        if (hsPath.startsWith('core/')) {
          ...
          if (module === 'hsm') {
            ...
            modulePath = path.join(hseosDir, module, rest);
          } else {
            // Other modules are under modules/
            modulePath = path.join(hseosDir, 'modules', module, rest);
          }
      ...
      case 'template': {
        ...
        if (templateDep.includes('{project-root}')) {
          // Remove {project-root} and treat as hseos-path
          templateDep = templateDep.replace('{project-root}', '');

          // Now resolve as a hseos path
          if (templateDep.startsWith('hseos/')) {
            const hsPath = templateDep.replace(/^hseos\//, '');
   ...
   */
  async resolveCommandToPath(hseosDir, command) {
    // Parse command format: @task-name or @agent-name or hseos/module/type/name

    if (command.startsWith('@task-')) {
      const taskName = command.slice(6);
      // Search all modules for this task
      for (const module of ['core', 'hsm', 'cis']) {
        const taskPath =
          module === 'core'
            ? path.join(hseosDir, 'core', 'tasks', `${taskName}.md`)
            : path.join(hseosDir, 'modules', module, 'tasks', `${taskName}.md`);
      ...
      for (const module of ['core', 'hsm', 'cis']) {
        const agentPath =
          module === 'core'
            ? path.join(hseosDir, 'core', 'agents', `${agentName}.md`)
            : path.join(hseosDir, 'modules', module, 'agents', `${agentName}.md`);
        if (await fs.pathExists(agentPath)) {
          return agentPath;
        }
      }
    } else if (command.startsWith('hseos/')) {
      ...
      if (parts.length >= 4) {
        ...
        const filePath =
          module === 'core' ? path.join(hseosDir, 'core', type, fileName) : path.join(hseosDir, 'modules', module, type, fileName);
   ...
   */
  async resolveTransitiveDependencies(hseosDir, directDeps) {
        ...
        const subDeps = await this.parseDependencies([
          {
            path: depPath,
            type: 'dependency',
            module: this.getModuleFromPath(hseosDir, depPath),
            name: path.basename(depPath),
          },
        ]);

        const resolvedSubDeps = await this.resolveDependencyPaths(hseosDir, subDeps);
   ...
   */
  getModuleFromPath(hseosDir, filePath) {
    const relative = path.relative(hseosDir, filePath);
   ...
   */
  organizeByModule(hseosDir, files) {
    const organized = {};

    for (const file of files) {
      const module = this.getModuleFromPath(hseosDir, file);
      if (!organized[module]) {
        organized[module] = {
          agents: [],
          tasks: [],
      ...
      if (file.includes('/src/core/') || file.includes('/src/bmm/')) {
        if (module === 'core') {
          moduleBase = path.join(hseosDir, 'src', 'core');
        } else if (module === 'hsm') {
          moduleBase = path.join(hseosDir, 'src', 'hsm');
        }
      } else {
        moduleBase = module === 'core' ? path.join(hseosDir, 'core') : path.join(hseosDir, 'modules', module);
      }
