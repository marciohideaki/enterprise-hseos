    ...
    try {
      ...
      if (options.directory) {
        ...
        const dirChoice = await prompts.select({
          message: 'Where do you want to uninstall HSEOS from?',
          choices: [
      ...
      if (!(await fs.pathExists(hseosDir))) {
        await prompts.log.warn('No HSEOS installation found.');
        process.exit(0);
      }
      ...
      await prompts.intro('HSEOS Uninstall');
      ...
      // Phase 3: HSEOS modules & data (last — other phases may need _hseos/)
      if (removeModules) {
        const s = await prompts.spinner();
        s.start(`Removing HSEOS modules & data (${installer.hseosFolderName}/)...`);
