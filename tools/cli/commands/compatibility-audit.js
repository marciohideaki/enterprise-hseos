...
function render(report) {
  const { callers, downstream, migration, telemetry } = report.evidence;
  return [
    ...
    `  downstream evidence bundle: ${downstream.ready ? 'available for human verification' : 'blocked'}`,
...
module.exports = {
  ...
  options: [
    ...
    ['--downstream-evidence <path>', 'Explicit downstream release-window evidence JSON'],
  ...
  async action(options) {
    ...
    const downstreamEvidencePath = options.downstreamEvidence ? path.resolve(options.downstreamEvidence) : undefined;
    const report = await auditCompatibility({ repositoryRoot, projectDirectory, downstreamEvidencePath, asOf });
