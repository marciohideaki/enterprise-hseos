...
class UI {
  ...
  async promptInstall(options = {}) {
    ...
    // Preflight: Check for legacy HSEOS v4 footprints immediately after getting directory
    ...
    let hasLegacyHseosFolder = false;
    let hseosDir = null;
    let legacyHseosPath = null;

    // First check for legacy .hseos folder (instead of _hseos)
    ...
    if (await fs.pathExists(confirmedDirectory)) {
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && (entry.name === '.hseos' || entry.name === 'hseos')) {
          hasLegacyHseosFolder = true;
          legacyHseosPath = path.join(confirmedDirectory, entry.name);
          hseosDir = legacyHseosPath;

          // Check if it has _cfg folder
          const cfgPath = path.join(legacyHseosPath, '_cfg');
    ...
    // If no .hseos or hseos found, check for current installations _hseos
    if (!hasLegacyHseosFolder) {
      const hseosResult = await installer.findHseosDir(confirmedDirectory);
      hseosDir = hseosResult.hseosDir;
      hasLegacyCfg = hseosResult.hasLegacyCfg;
    }

    // Handle legacy .hseos or _cfg folder - these are very old (v4 or alpha)
    // Show version warning instead of offering conversion
    if (hasLegacyHseosFolder || hasLegacyCfg) {
      await prompts.log.warn('LEGACY INSTALLATION DETECTED');
      await prompts.note(
        'Found a ".hseos"/"hseos" folder, or a legacy "_cfg" folder under the hseos folder -\n' +
          'this is from an old HSEOS version that is out of date for automatic upgrade,\n' +
          ...
          '  1. Delete your current HSEOS installation folder (.hseos or hseos)\n' +
          '  2. Run a fresh installation\n\n' +
          'If you do not want to start fresh, you can attempt to proceed beyond this\n' +
          'point IF you have ensured the hseos folder is named _hseos, and under it there\n' +
          'is a _config folder. If you have a folder under your hseos folder named _cfg,\n' +
      ...
      try {
        // Handle .hseos folder
        if (hasLegacyHseosFolder) {
          const newHseosPath = path.join(confirmedDirectory, '.hseos');
          await fs.move(legacyHseosPath, newHseosPath);
          hseosDir = newHseosPath;
          s.stop(`Renamed "${path.basename(legacyHseosPath)}" to ".hseos"`);
        }

        // Handle _cfg folder (either from .hseos or standalone)
        const cfgPath = path.join(hseosDir, '_cfg');
        if (await fs.pathExists(cfgPath)) {
          s.start('Renaming configuration folder...');
          const newCfgPath = path.join(hseosDir, '_config');
    ...
    // Check if there's an existing HSEOS installation (after any folder renames)
    const hasExistingInstall = await fs.pathExists(hseosDir);
    ...
    if (hasExistingInstall) {
      ...
      const { existingInstall, hseosDir } = await this.getExistingInstallation(confirmedDirectory);
      const packageJsonPath = path.join(__dirname, '../../../package.json');
      const currentVersion = require(packageJsonPath).version;
      const installedVersion = existingInstall.version || 'unknown';

      // Check if version is pre beta
      const shouldProceed = await this.showLegacyVersionWarning(installedVersion, currentVersion, path.basename(hseosDir), options);
      ...
      // Common actions
      choices.push({ name: 'Modify HSEOS Installation', value: 'update' });
      ...
      if (actionType === 'update') {
        ...
        if (options.customContent) {
          ...
          if (changeCustomModules) {
            ...
            const { Installer } = require('../installers/lib/core/installer');
            const installer = new Installer();
            const { hseosDir } = await installer.findHseosDir(confirmedDirectory);
  ...
  async promptToolSelection(projectDir, options = {}) {
    // Check for existing configured IDEs - use findHseosDir to detect custom folder names
    const { Detector } = require('../installers/lib/core/detector');
    const { Installer } = require('../installers/lib/core/installer');
    const detector = new Detector();
    const installer = new Installer();
    const hseosResult = await installer.findHseosDir(projectDir || process.cwd());
    const hseosDir = hseosResult.hseosDir;
   ...
   * @returns {Object} Object with existingInstall, installedModuleIds, and hseosDir
   */
  async getExistingInstallation(directory) {
    ...
    const detector = new Detector();
    const installer = new Installer();
    const hseosDirResult = await installer.findHseosDir(directory);
    const hseosDir = hseosDirResult.hseosDir;
    const existingInstall = await detector.detect(hseosDir);
    const installedModuleIds = new Set(existingInstall.modules.map((mod) => mod.id));

    return { existingInstall, installedModuleIds, hseosDir };
  }
  ...
  async selectAllModules(installedModuleIds = new Set()) {
    ...
    allOptions.push({ label: 'HSEOS Core Module', value: 'core', hint: 'Core configuration and shared resources' });
    ...
    // Group 2: HSEOS Official Modules (type: hseos-org)
    const officialModules = [];
    for (const mod of externalModules) {
      if (mod.type === 'hseos-org') {
  ...
  async getDefaultModules(installedModuleIds = new Set()) {
    ...
    // If no defaults found, use 'hsm' as the fallback default
    if (defaultModules.length === 0) {
      defaultModules.push('hsm');
    }
  ...
  async displayDirectoryInfo(directory) {
    ...
    if (dirExists) {
      ...
      if (stats.isDirectory()) {
        ...
        if (files.length > 0) {
          // Check for any hseos installation (any folder with _config/manifest.yaml)
          const { Installer } = require('../installers/lib/core/installer');
          const installer = new Installer();
  ...
  async getConfiguredIdes(directory) {
    ...
    const installer = new Installer();
    const hseosResult = await installer.findHseosDir(directory);
    const existingInstall = await detector.detect(hseosResult.hseosDir);
  ...
  async handleCustomModulesInModifyFlow(directory, selectedModules) {
    ...
    const { Installer } = require('../installers/lib/core/installer');
    const installer = new Installer();
    const { hseosDir } = await installer.findHseosDir(directory);
   ...
   * @param {string} hseosFolderName - Name of the HSEOS folder
   * @returns {Promise<boolean>} True if user wants to proceed, false if they cancel
   */
  async showLegacyVersionWarning(installedVersion, currentVersion, hseosFolderName, options = {}) {
    ...
    let warningContent;
    if (installedVersion === 'unknown') {
      warningContent = 'Unable to detect your installed HSEOS version.\n' + 'This appears to be a legacy or unsupported installation.';
    } else {
      warningContent =
    ...
    warningContent +=
      ...
      '  1. Delete your current HSEOS installation folder\n' +
      `     (the "${hseosFolderName}/" folder in your project)\n` +
      '  2. Run a fresh installation\n\n' +
      'Benefits of a fresh install:\n' +
      '  \u2022 Cleaner configuration without legacy artifacts\n' +
      '  \u2022 All new features properly configured\n' +
    ...
    if (proceed === 'cancel') {
      await prompts.note(
        `1. Delete the "${hseosFolderName}/" folder in your project\n` + "2. Run 'hseos install' again",
  ...
  async displayStatus(statusData) {
    const { installation, modules, availableUpdates, hseosDir } = statusData;
    ...
    if (availableUpdates.length > 0) {
      ...
      await prompts.log.message('Run \'hseos install\' and select "Quick Update" to update');
    } else {
