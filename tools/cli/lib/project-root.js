...
/**
 * Find the HSEOS project root directory by looking for package.json
 * or specific HSEOS markers
 */
function findProjectRoot(startPath = __dirname) {
  let currentPath = path.resolve(startPath);

  // Keep going up until we find package.json with hseos
  while (currentPath !== path.dirname(currentPath)) {
    ...
    if (fs.existsSync(packagePath)) {
      try {
        ...
        // Check if this is the HSEOS project
        if (pkg.name === 'hseos' || fs.existsSync(path.join(currentPath, 'src', 'core'))) {
          return currentPath;
        }
      } catch {
        // Continue searching
 ...
 * hsm is a built-in module directly under src/
...
function getModulePath(moduleName, ...segments) {
  ...
  if (moduleName === 'hsm') {
    return getSourcePath('hsm', ...segments);
  }
