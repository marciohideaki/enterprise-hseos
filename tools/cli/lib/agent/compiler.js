/**
 * HSEOS Agent Compiler
 * Transforms agent YAML to compiled XML (.md) format
 * Uses the existing HSEOS builder infrastructure for proper formatting
 */
...
function buildFrontmatter(metadata, agentName) {
  ...
  const description = metadata.title || 'HSEOS Agent';
...
function buildMenuXml(menuItems) {
  ...
  xml += `    <item cmd="PM or fuzzy match on party-mode" exec="{project-root}/.hseos/core/workflows/party-mode/workflow.md">[PM] Start Party Mode</item>\n`;
