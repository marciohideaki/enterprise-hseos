...
/**
 * Find HSEOS config file in project
 * @param {string} startPath - Starting directory to search from
 * @returns {Object|null} Config data or null
 */
function findHseosConfig(startPath = process.cwd()) {
  // Look for common HSEOS folder names
  const possibleNames = ['.hseos'];
  ...
  for (const name of possibleNames) {
    ...
    if (fs.existsSync(configPath)) {
      ...
      return {
        ...config,
        hseosFolder: path.join(startPath, name),
...
/**
 * Resolve path variables like {project-root} and {hseos-folder}
 * @param {string} pathStr - Path with variables
 * @param {Object} context - Contains projectRoot, hseosFolder
 * @returns {string} Resolved path
 */
function resolvePath(pathStr, context) {
  return pathStr.replaceAll('{project-root}', context.projectRoot).replaceAll('{hseos-folder}', context.hseosFolder);
}
...
/**
 * Detect if a path is within an HSEOS project
 * @param {string} targetPath - Path to check
 * @returns {Object|null} Project info with hseosFolder and cfgFolder
 */
function detectHseosProject(targetPath) {
  let checkPath = path.resolve(targetPath);
  const root = path.parse(checkPath).root;

  // Walk up directory tree looking for HSEOS installation
  while (checkPath !== root) {
    const possibleNames = ['.hseos'];
    for (const name of possibleNames) {
      const hseosFolder = path.join(checkPath, name);
      const cfgFolder = path.join(hseosFolder, '_config');
...
async function createIdeSlashCommands(projectRoot, agentName, agentPath, metadata) {
  ...
  const manifestPath = path.join(projectRoot, '.hseos', '_config', 'manifest.yaml');
