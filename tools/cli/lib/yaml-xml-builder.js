...
class YamlXmlBuilder {
  ...
  async convertToXml(agentYaml, buildMetadata = {}) {
    ...
    if (buildMetadata.forWebBundle) {
      ...
      xml += `description: "${metadata.title || 'HSEOS Agent'}"\n`;
      xml += '---\n\n';
      xml +=
  ...
  async buildFromYaml(agentYamlPath, customizeYamlPath = null, options = {}) {
    ...
    // or /path/to/hseos/hsm/agents/pm.yaml -> bmm
    ...
    const hseosIndex = pathParts.indexOf('hseos');
    const srcIndex = pathParts.indexOf('src');

    if (modulesIndex !== -1 && pathParts[modulesIndex + 1]) {
      // Path contains /modules/{module}/
      module = pathParts[modulesIndex + 1];
    } else if (hseosIndex !== -1 && pathParts[hseosIndex + 1]) {
      // Path contains /hseos/{module}/
      const potentialModule = pathParts[hseosIndex + 1];
      // Check if it's a known module, not 'agents' or '_config'
      if (['hsm', 'bmb', 'cis', 'core'].includes(potentialModule)) {
      ...
      if (potentialModule === 'hsm' || potentialModule === 'core') {
        module = potentialModule;
      }
    }
