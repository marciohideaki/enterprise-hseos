...
const AgentPartyGenerator = {
  ...
  generateAgentParty(agentDetails, options = {}) {
    ...
    // Build XML content
    let xmlContent = `<!-- Powered by HSEOS-CORE™ -->
<!-- Agent Manifest - Generated during HSEOS ${forWeb ? 'bundling' : 'installation'} -->
<!-- This file contains a summary of all ${forWeb ? 'bundled' : 'installed'} agents for quick reference -->
<manifest id="hseos/_config/agent-manifest.csv" version="1.0" generated="${new Date().toISOString()}">
  <description>
    Complete roster of ${forWeb ? 'bundled' : 'installed'} HSEOS agents with summarized personas for efficient multi-agent orchestration.
...
`;
    ...
    for (const [module, agents] of Object.entries(agentsByModule)) {
      ...
      const moduleTitle =
        module === 'hsm' ? 'BMM Module' : module === 'cis' ? 'CIS Module' : module === 'core' ? 'Core Module' : 'Custom Module';
  ...
  extractAgentDetails(content, moduleName, agentName) {
    try {
      ...
      return {
        id: `hseos/${moduleName}/agents/${agentName}.md`,
