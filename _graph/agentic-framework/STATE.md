...
- **Next reversible action:** collect complete zero-use G9 days while obtaining owner disposition for the three state-only repositories classified as legacy; keep downstream migration, final stable audit and operational release changes gated
...
- **A13 downstream preparation:** a strict local packer can derive the G9 plugin/installer evidence bundle; local Git discovery identifies four candidates and no local plugin-v1 consumer, but the three legacy-classified layouts contain only state/run artifacts and are not proven v4 installations; owner membership disposition, a complete release-bound registry, remote verification and real R/R+1 artifacts remain absent
