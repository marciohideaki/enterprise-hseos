...
- **Validation:** A13 bound kernel `6/6`; reference `8/8`; provider binding `8/8`; full strict gate passed with 0 failures and 1 unrelated historical warning
...
- **Next reversible action:** expose the bound kernel through an `ai-jail`-attesting supervisor and CLI surface; real probing remains sandbox-gated
...
- **Operational constraint:** the bound runtime accepts only injected `ai-jail` lockdown attestations and is not yet routed by the public CLI; actual sandbox/provider environment, G9 compatibility evidence and separate human authorization remain prerequisites for activation
