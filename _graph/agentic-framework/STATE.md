...
- **Next reversible action:** collect complete zero-use G9 days while obtaining an organization-wide complete registry and canonical `platform-gitops` binding; keep downstream migration, final stable audit and operational release changes gated
...
- **A13 downstream preparation:** a strict local packer can derive the G9 plugin/installer evidence bundle; authenticated reads verify all four exact remote Git objects and exact advertised refs for three, while the named `platform-gitops` ref is absent; the three legacy-classified layouts contain only state/run artifacts and are not proven v4 installations; organization-wide completeness, a complete release-bound registry, canonical `platform-gitops` binding and real R/R+1 artifacts remain absent
