...
- **Next reversible action:** assemble the hash-bound DeepSeek one-shot provider under the required sandbox supervisor and expose only the supported public run action
...
- **Operational constraint:** the public candidate route requires real `ai-jail` readiness and cannot accept a caller-supplied attestation; this host remains unavailable for a live sandbox/provider smoke; G9 compatibility evidence and separate human authorization remain prerequisites for activation
