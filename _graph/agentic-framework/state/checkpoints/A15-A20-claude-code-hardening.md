# A15–A20 — Claude Code Learning Hardening Closeout

**Artifact type:** Governed implementation checkpoint  
**Scope:** Seven proposals selected from the Claude Code reference analysis  
...
|   6 | Egress credential transformer        | Partially adopted | Narrow host-owned injection at a pinned Unix-socket route is retained                                                            | `provider-egress-broker`; `test:provider-egress-broker`                   |
...
- Integrated conformance reads the external canonical catalog, resolves every suite/module/export, scans an explicit core allowlist for vendor branches, and executes the operational migration catalog through schema v4.
- Independent A21 revalidation: `READY`, with no blocker/high/medium finding after the initial circular-test finding was corrected.
- Final full quality gate: 0 failures and 1 historical placeholder warning (`.logs/validation/gate-20260824T081928.log`).

## Uniformity and rollback

The kernel owns policy, configuration, isolation, messaging, worker state, egress authority and trace lineage. Codex, Claude Code, DeepSeek and future clients remain substitutable adapters and cannot weaken these rules. Each implementation remains in an isolated reversible commit. Operational schema remains v4; pending fixture schemas, G9 observation, final stable audit and explicit human cutover remain separate gates.
